...
/**************************************************************************/
#ifdef EN_USB_LOG
static const char* Get_func_str(uint32_t req_type, uint32_t v)
{
  if ((req_type & 0x1F) == 0)
  {
    switch (v)
    {
    case 0 : return "Dev.Req. GET_STATUS (0)";
    case 1 : return "Dev.Req. CLEAR_FEATURE (1)";
    case 3 : return "Dev.Req. SET_FEATURE (3)";
    case 5 : return "Dev.Req. SET_ADDRESS (5)";
    case 6 : return "Dev.Req. GET_DESCRIPTOR (6)";
    case 7 : return "Dev.Req. SET_DESCRIPTOR (7)";
    case 8 : return "Dev.Req. GET_CONFIGURATION (8)";
    case 9 : return "Dev.Req. SET_CONFIGURATION (9)";
    default: return "???";
    }
  }
  if ((req_type & 0x1F) == 1)
  {
    switch (v)
    {
    case 0    : return "Intf.Req. GET_STATUS (0)";
    case 1    : return "Intf.Req. CLEAR_FEATURE (1)";
    case 3    : return "Intf.Req. SET_FEATURE (3)";
    case 0x0A : return "Intf.Req. GET_INTERFACE (0x0A)";
    case 0x0B : return "Intf.Req. SET_INTERFACE (0x0B)";
    default: return "???";
    }
  }
  if ((req_type & 0x1F) == 2)
  {
    switch (v)
    {
    case 0    : return "EP Req. GET_STATUS (0)";
    case 1    : return "EP Req. CLEAR_FEATURE (1)";
    case 3    : return "EP Req. SET_FEATURE (3)";
    case 0x0C : return "EP Req. SYNCH_FRAME (0x0C)";
    default: return "???";
    }
  }
  return "???";
...
UCHAR *                 saved_request_data_pointer;
    ...
    *(setup_request + UX_SETUP_REQUEST) =       transfer_request -> ux_transfer_request_function;
    ...
    USBLOG(" EP addr:%d %s", endpoint->ux_endpoint_device->ux_device_address, Get_func_str(transfer_request->ux_transfer_request_type, transfer_request->ux_transfer_request_function));
    ...
    USBLOG(" CC:%d",transfer_request -> ux_transfer_request_completion_code);
    ...
    if (saved_requested_length)
    {
        ...
        USBLOG(" Pack len:%d dir:%d",transfer_request -> ux_transfer_request_packet_length, (transfer_request -> ux_transfer_request_type & UX_REQUEST_DIRECTION) == UX_REQUEST_IN ? 1 : 0);
...
#ifdef EN_USB_LOG
        if (transfer_request -> ux_transfer_request_function == 6)
        {
          char     str[16*3+1];
          char     *ptr;
          uint32_t i=0;
          do
          {
            ...
            {
              sprintf(ptr,"%02X ", transfer_request->ux_transfer_request_data_pointer[i]);
              ptr+=3;
              i++;
              if (i >=  transfer_request->ux_transfer_request_actual_length) break;
            }
            USBLOG(str);
            if (i >=  transfer_request->ux_transfer_request_actual_length) break;
          } while(1);
