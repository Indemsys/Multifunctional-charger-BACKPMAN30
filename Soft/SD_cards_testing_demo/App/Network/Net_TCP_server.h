...
#ifndef NET_TCP_SERVER_PACKET_PAYLOAD
#define NET_TCP_SERVER_PACKET_PAYLOAD        300
#endif

  /* Define the size of the Telnet Server packet pool. This will allow room for about 5-6 packets of 300 byte payload. */
...
#define NET_TCPSRV_ANY_EVENT                 0xFF        /* Any TELNET event                                     */
...
#define NET_TCPSRV_INVALID_PARAMETER         0xF5        /* Invalid non pointer input to Telnet function         */
#define NET_TCPSRV_NO_PACKET_POOL            0xF6        /*  server packet pool not set                    */
  ...
  typedef struct
  {
    ...
    UCHAR                             en_options_negotiation; // Флаг разрешения процедуры согласования опций по протоколу Telnet
    ULONG                             activity_timeout_val;   // Время отсутствия приеме после кторого соединение разрывается. если 0 то это время не контролируется
