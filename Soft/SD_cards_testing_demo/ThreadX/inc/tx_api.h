...
typedef struct TX_TIMER_INTERNAL_STRUCT
{
  ...
  /* Define the next and previous internal link pointers for ready_to_send
     internal timers.  */
...
UINT        _txr_timer_info_get(TX_TIMER *timer_ptr, CHAR **name, UINT *ready_to_send, ULONG *remaining_ticks,
                ULONG *reschedule_ticks, TX_TIMER **next_timer);
#endif
...
#define TX_SAFETY_CRITICAL_EXCEPTION_HANDLER    VOID  _tx_safety_critical_exception_handler(CHAR *g_file_name, INT line_number, UINT status) \
                                                { \
                                                    while(1) \
                                                    { \
                                                    } \
