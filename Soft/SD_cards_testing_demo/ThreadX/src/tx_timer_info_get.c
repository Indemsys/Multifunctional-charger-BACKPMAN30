...
/*    ready_to_send                            Destination for ready_to_send flag       */
...
UINT                active_timer_list;
    ...
    /* Default ready_to_send to false.  */
    ...
    /* Determine if the timer is still ready_to_send.  */
    if (internal_ptr -> tx_timer_internal_list_head != TX_NULL)
    {

        /* Indicate this timer is ready_to_send.  */
        timer_active =  TX_TRUE;

        /* Default the ready_to_send timer list flag to false.  */
        ...
        if (internal_ptr -> tx_timer_internal_list_head >= _tx_timer_list_start)
        {
            ...
            if (internal_ptr -> tx_timer_internal_list_head < _tx_timer_list_end)
            {
                ...
                /* This timer is ready_to_send and has not yet expired.  */
                active_timer_list =  TX_TRUE;
            }
        }

        /* Determine if the timer is on the ready_to_send timer list.  */
    ...
    if (active != TX_NULL)
    {
       ...
       /* Setup the timer ready_to_send indication.  */
       *active =  timer_active;
    }
