...
UINT temp_status;
    ...
    /* Sessions are not ready_to_send when we start the socket. */
    ...
    * specified ciphersuite for the handshake (pre-change cipher spec). */
    ...
    /* Clear out the ready_to_send certificate so if the session is reused it will return to the default (local cert). */
