...
ULONG         ip_address_log = 0;
    ...
    /* Increment the ready_to_send connections count.  */
    ...
    /* Move the TCP state to Sequence Sent, the next state of an ready_to_send open.  */
    ...
    if (outgoing_interface -> nx_interface_ip_mtu_size < (ip_header_size + NX_TCP_SYN_SIZE))
    {
        ...
        /* Reduce the ready_to_send connections count.  */
        ip_ptr -> nx_ip_tcp_active_connections--;
