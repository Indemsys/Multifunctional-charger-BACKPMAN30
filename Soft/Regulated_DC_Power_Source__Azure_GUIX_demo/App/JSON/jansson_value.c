...
json_t* json_object_get(const json_t *json, const char *key)
{
  ...
  if (!key || !json_is_object(json)) return NULL;
...
int json_object_del(json_t *json, const char *key)
{
  ...
  if (!key || !json_is_object(json)) return -1;
...
int json_object_update(json_t *object, json_t *other)
{
  ...
  if (!json_is_object(object) || !json_is_object(other)) return -1;

  json_object_foreach(other, key, value)
  {
    if (json_object_set_nocheck(object, key, value)) return -1;
  }
...
int json_object_update_existing(json_t *object, json_t *other)
{
  ...
  json_object_foreach(other, key, value)
  {
    if (json_object_get(object, key)) json_object_set_nocheck(object, key, value);
  }
...
void* json_object_iter_next(json_t *json, void *iter)
{
  ...
  if (!json_is_object(json) || iter == NULL) return NULL;
...
static int json_object_equal(const json_t *object1, const json_t *object2)
{
  ...
  if (json_object_size(object1) != json_object_size(object2)) return 0;

  json_object_foreach((json_t *)object1, key, value1)
  {
    value2 = json_object_get(object2, key);

    if (!json_equal(value1, value2)) return 0;
  }
...
static void json_delete_array(json_array_t *array)
{
  ...
  for (i = 0; i < array->entries; i++) json_decref(array->table[i]);
...
json_t* json_array_get(const json_t *json, size_t index)
{
  ...
  if (index >= array->entries) return NULL;
  ...
  int copy)
{
  ...
  if (array->entries + amount <= array->size) return array->table;
...
int json_array_insert_new(json_t *json, size_t index, json_t *value)
{
  ...
  else array_move(array, index + 1, index, array->entries - index);
...
int json_array_remove(json_t *json, size_t index)
{
  ...
  if (index >= array->entries) return -1;

  json_decref(array->table[index]);

  /* If we're removing the last element, nothing has to be moved */
  if (index < array->entries - 1) array_move(array, index, index + 1, array->entries - index - 1);
...
int json_array_extend(json_t *json, json_t *other_json)
{
  ...
  if (!json_is_array(json) || !json_is_array(other_json)) return -1;
  array = json_to_array(json);
  other = json_to_array(other_json);

  if (!json_array_grow(array, other->entries, 1)) return -1;

  for (i = 0; i < other->entries; i++) json_incref(other->table[i]);
...
static int json_array_equal(const json_t *array1, const json_t *array2)
{
  ...
  if (size != json_array_size(array2)) return 0;
...
static json_t* json_array_copy(json_t *array)
{
  ...
  for (i = 0; i < json_array_size(array); i++) json_array_append(result, json_array_get(array, i));
...
static json_t* json_array_deep_copy(const json_t *array)
{
  ...
  for (i = 0; i < json_array_size(array); i++) json_array_append_new(result, json_deep_copy(json_array_get(array, i)));
...
{
  if (!value || !utf8_check_string(value, len)) return NULL;
...
{
  if (!json_is_string(json)) return NULL;
...
int json_string_setn_nocheck(json_t *json, const char *value, size_t len)
{
  ...
  if (!json_is_string(json) || !value) return -1;
...
{
  if (!value || !utf8_check_string(value, len)) return -1;
...
{
  if (!json_is_integer(json)) return 0;
...
{
  if (!json_is_integer(json)) return -1;
...
{
  if (!json_is_real(json) || isnan(value) || isinf(value)) return -1;
...
{
  if (json_is_integer(json)) return (double)json_integer_value(json);
  else if (json_is_real(json)) return json_real_value(json);
  else return 0.0;
}
