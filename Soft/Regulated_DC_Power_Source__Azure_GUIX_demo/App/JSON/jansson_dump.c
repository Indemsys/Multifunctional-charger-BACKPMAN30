...
static int dump_to_file(const char *buffer, size_t size, void *data)
{
    ...
    if(fx_file_write(f, (void*)buffer, size) != FX_SUCCESS)
        return -1;
    return 0;
}
...
int json_dumpf(const json_t *json, FX_FILE *f, size_t flags)
{
    return json_dump_callback(json, dump_to_file, (void *)f, flags);
}
...
int json_dump_file(const json_t *json, const char *path, size_t flags)
{
    ...
    if(fx_file_open(FILE_MEDIA,&f, (CHAR*)path,(FX_OPEN_FOR_WRITE)) != FX_SUCCESS)
        return -1;

    result = json_dumpf(json, &f, flags);

    if(fx_file_close(&f) != FX_SUCCESS)
        return -1;
