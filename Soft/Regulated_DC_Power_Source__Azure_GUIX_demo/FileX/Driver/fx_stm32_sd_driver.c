...
ALIGN_32BYTES(static UCHAR scratch[FX_STM32_SD_DEFAULT_SECTOR_SIZE]);
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param start_sector
  \param num_sectors
  \param use_scratch_buffer - use_scratch_buffer to enable scratch buffer usage or not.
...
UINT SD_FX_read_data(FX_MEDIA *media_ptr, ULONG start_sector, UINT num_sectors, UINT use_scratch_buffer)
{
  ...
  if (use_scratch_buffer)
  {
    ...
    for (i = 0; i < num_sectors; i++)
    {
      ...
      status = MX_SDMMC1_SD_ReadBlocks_DMA(scratch, (UINT)start_sector++, 1);
      ...
      SCB_InvalidateDCache_by_Addr((void *)scratch, (int32_t)FX_STM32_SD_DEFAULT_SECTOR_SIZE);
#endif
...
UINT SD_FX_write_data(FX_MEDIA *media_ptr, ULONG start_sector, UINT num_sectors, UINT use_scratch_buffer)
{
  ...
  if (use_scratch_buffer)
  {
    ...
    for (i = 0; i < num_sectors; i++)
    {
      ...
      SCB_CleanDCache_by_Addr((uint32_t *)scratch, (int32_t)FX_STM32_SD_DEFAULT_SECTOR_SIZE);
#endif

      status = MX_SDMMC1_SD_WriteBlocks_DMA(scratch, (UINT)start_sector++, 1);
