...
/* Define the current state variable.  When this value is 0, a thread
   ...
   interrupt or initialization processing is ready_to_send.  This variable is
...
/* Define the priority map ready_to_send bit map that specifies which of the previously
...
/* Define the 32-bit preempt priority bit maps.  There is one preempt bit map 
   ...
   level that had preemption-threshold ready_to_send to protect against preemption of a
   range of relatively higher priority threads.  */

ULONG           _tx_thread_preempted_maps[TX_MAX_PRIORITIES/32];


/* Define the preempt map ready_to_send bit map that specifies which of the previously
