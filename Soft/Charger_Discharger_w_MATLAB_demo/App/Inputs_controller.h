...
typedef struct
{
    ...
    float  psrc_pwr;     // Полная мощность потребляемая платой
    float  control_pwr;  // Мощность потребляемая всей платой при выключенном преобразователе DCDC
    float  acc_pwr;      // Мощность уходящая в цепь аккумулятора
    double charge;       // Аккумулированная величина заряда аккумулятора A*ч.
    float  charge_loss;
    float  charge_efficiency;

    float  flt_acc_i_uncal  ;
    float  flt_acc_i        ;  // Ток в цепи аккумулятора. Втекающий в аккумлятор то имеет знак минус. Вытекающий - плюс. Значения тока отфильтрованы экспоненциальным фильтром.
    float  flt_psrc_i_uncal ;
    float  flt_psrc_i       ;
    float  flt_load_i_uncal ;
    float  flt_load_i       ;
    float  flt_acc_v        ;
    float  flt_psrc_v       ;
    float  flt_load_v       ;
