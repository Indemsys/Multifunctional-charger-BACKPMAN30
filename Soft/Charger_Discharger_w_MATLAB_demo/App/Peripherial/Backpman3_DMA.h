...
#define ADC_RES_READY           BIT(0)   // Каждые ADC_RESULTS_PERIOD_US установливаем флаг
#define ADC_SYSV_TOO_LOW        BIT(1)
#define LOG_ADC_RES             BIT(2)
#define APP_DO_SYSTEM_RESTART   BIT(3)

extern TX_EVENT_FLAGS_GROUP   adc_flag;

void DMA1_from_ADC_init(void);
void DMA1_from_SPI4_init(void);
void DMA1_mem_to_SPI4_start(volatile uint8_t *mem_addr, uint16_t len, uint8_t bits, uint8_t en_inc);
