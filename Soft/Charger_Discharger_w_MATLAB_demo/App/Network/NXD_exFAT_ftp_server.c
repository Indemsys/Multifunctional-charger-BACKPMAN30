        ...
        if (client_req_ptr -> nx_ftp_client_request_control_socket.nx_tcp_socket_receive_queue_count)
        {
            ...
            case NX_FTP_CWD:
            {
                ...
                if (ftp_command == NX_FTP_CDUP)
                {
                 ...
                 //    Данный фрагмент не работает при использовании exFAT
                 ...
                 //    buffer_ptr[2] = NX_NULL;
                  ...
                  // Получаем текущий путь, удаляем из него окончание с именем директории и назначаем отредактированный путь
                  // Таким образом передвигаемся верх на одну директорию
                  CHAR *curr_path;
                  fx_directory_local_path_get(ftp_server_ptr -> nx_ftp_server_media_ptr, &curr_path);
                  int plen = strlen(curr_path);
                  int k = plen-1;
                  for (;k>=0;k--) if (curr_path[k] == '\\') break;

                  if (k>0) curr_path[k] = 0;
                  else if (k==0) curr_path[k+1] = 0;
                  status =  fx_directory_local_path_set(ftp_server_ptr -> nx_ftp_server_media_ptr, &(client_req_ptr -> nx_ftp_client_local_path), curr_path);
                }
                ...
                else
                {
                    ...
                    if (packet_ptr -> nx_packet_length == 0)
                    {
                        ...
                        /* Now send an error response to the client.  */
                        _nx_ftp_server_response(&(client_req_ptr -> nx_ftp_client_request_control_socket), packet_ptr,NX_FTP_CODE_BAD_FILE, "Change Dir Fail");
                        break;
                    }
                    ...
                    status =  fx_directory_local_path_set(ftp_server_ptr -> nx_ftp_server_media_ptr, &(client_req_ptr -> nx_ftp_client_local_path), (CHAR *) buffer_ptr);
