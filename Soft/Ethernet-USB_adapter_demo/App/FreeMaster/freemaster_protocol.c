...
FMSTR_TRANSPORT_INTF *fmstr_drv_ptr;
...
FMSTR_BOOL FMSTR_Init(void *drv_ptr)
{
    FMSTR_BOOL status = FMSTR_TRUE;

    if (drv_ptr == 0) return FMSTR_FALSE;
    fmstr_drv_ptr = (FMSTR_TRANSPORT_INTF*)drv_ptr;
    ...
    if (fmstr_drv_ptr->Init() == FMSTR_FALSE)
    {
        status = FMSTR_FALSE;
    }
...
void FMSTR_SendResponse(FMSTR_BPTR response, FMSTR_SIZE length, FMSTR_U8 statusCode, FMSTR_SESSION *session)
{
    ...
    fmstr_drv_ptr->SendResponse(response, length, statusCode, session->identification);
}
