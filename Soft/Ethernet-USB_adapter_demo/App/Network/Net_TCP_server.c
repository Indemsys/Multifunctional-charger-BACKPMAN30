...
/*-----------------------------------------------------------------------------------------------------
  ...
  \return UINT
-----------------------------------------------------------------------------------------------------*/
UINT Net_tcp_server_create(NET_TCP_SERVER *server_ptr, T_tcpsrv_setup *setup_ptr)
{
  UINT            status;
  ...
  status =  tx_thread_create(&(server_ptr->server_thread), "TCP Server",
  ...
  status =  tx_event_flags_create(&(server_ptr->event_flags), "TCP Server");
  ...
  status +=  nx_tcp_socket_create(&(server_ptr->ip),
                                  &(server_ptr->socket),
                                  "TCP Server",
                                  NET_TCPSRV_TOS,
...
UINT  Net_tcp_server_delete(NET_TCP_SERVER *server_ptr)
{
  ...
  nx_tcp_socket_delete(&(server_ptr->socket));
  nx_tcp_server_socket_unlisten(&(server_ptr->ip), server_ptr->server_port);
...
UINT  Net_tcp_server_disconnect(NET_TCP_SERVER *server_ptr)
{
  ...
  {
    status =  nx_tcp_server_socket_relisten(&(server_ptr->ip), server_ptr->server_port,&(server_ptr->socket)); // Relisten on this socket.
...
UINT  Net_tcp_server_start(NET_TCP_SERVER *server_ptr)
{
  ...
  status =  nx_tcp_server_socket_listen(&(server_ptr->ip), server_ptr->server_port, &(server_ptr->socket), 1, _Connection_callback); // Start listening on the socket.
...
UINT  Net_tcp_server_stop(NET_TCP_SERVER *server_ptr)
{
  ...
  nx_tcp_server_socket_unlisten(&(server_ptr->ip), server_ptr->server_port);     // Unlisten on the port.
  return (NX_SUCCESS);
}
...
VOID  Net_tcp_server_connecting(NET_TCP_SERVER *server_ptr)
{
      ...
      (server_ptr->socket.nx_tcp_socket_connect_port))
  {
    ...
    else
    {
      ...
      server_ptr->activity_timeout =  NET_TCPSRV_ACTIVITY_TIMEOUT;
  ...
  if (server_ptr->socket.nx_tcp_socket_state == NX_TCP_CLOSED)
  {
    ...
    status =  nx_tcp_server_socket_relisten(&(server_ptr->ip), server_ptr->server_port,&(server_ptr->socket));
...
VOID  Net_tcp_server_timeout_processing(NET_TCP_SERVER *server_ptr)
{
  ...
  if (server_ptr->activity_timeout)
  {
    ...
    if (server_ptr->activity_timeout == 0) // Determine if this entry has exceeded the activity timeout.
    {
      ...
      nx_tcp_server_socket_relisten(&(server_ptr->ip), server_ptr->server_port, &(server_ptr->socket));
...
VOID  Net_tcp_server_data_receiving(NET_TCP_SERVER *server_ptr)
{
  ...
  {
    server_ptr->activity_timeout =  NET_TCPSRV_ACTIVITY_TIMEOUT;                    // Reset the client request activity timeout.
