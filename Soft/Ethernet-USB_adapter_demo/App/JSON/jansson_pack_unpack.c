...
static json_t* pack_object(scanner_t *s, va_list *ap)
{
  ...
  while (token(s) != '}')
  {
    ...
    if (s->has_error) json_decref(value);
...
static json_t* pack_string(scanner_t *s, va_list *ap)
{
  ...
  t = token(s);
  optional = t == '?' || t == '*';
  if (!optional) prev_token(s);

  str = read_string(s, ap, "string",&len,&ours, optional);

  if (!str) return t == '?' && !s->has_error ? json_null(): NULL;
...
static json_t* pack_object_inter(scanner_t *s, va_list *ap, int need_incref)
{
  ...
  if (ntoken != '?' && ntoken != '*') prev_token(s);

  json = va_arg(*ap, json_t *);

  if (json) return need_incref ? json_incref(json): json;
...
static int unpack_object(scanner_t *s, json_t *root, va_list *ap)
{
  ...
  */
  ...
  while (token(s) != '}')
  {
    ...
    if (unpack(s, value, ap)) goto out;

    hashtable_set(&key_set, key, json_null());
    next_token(s);
  }

  if (strict == 0 && (s->flags & JSON_STRICT)) strict = 1;
    ...
    if (gotopt || json_object_size(root) != key_set.size)
    {
      json_object_foreach(root, key, value)
      {
        if (!hashtable_get(&key_set, key))
        {
          ...
          if (!keys_res) keys_res = strbuffer_append_bytes(&unrecognized_keys, key, strlen(key));
        }
      }
    }
    if (unpacked)
...
static int unpack(scanner_t *s, json_t *root, va_list *ap)
{
    ...
    if (!(s->flags & JSON_VALIDATE_ONLY))
    {
      ...
      if (root)
      {
        *str_target = json_string_value(root);
        if (len_target) *len_target = json_string_length(root);
      }
    }
    return 0;
  ...
  case 'i':
    ...
    if (!(s->flags & JSON_VALIDATE_ONLY))
    {
      ...
      if (root) *target = (int)json_integer_value(root);
    }
  ...
  case 'I':
    ...
    if (!(s->flags & JSON_VALIDATE_ONLY))
    {
      ...
      if (root) *target = json_integer_value(root);
    }
  ...
  case 'b':
    ...
    {
      int *target = va_arg(*ap, int *);
      if (root) *target = json_is_true(root);
    }
  ...
  case 'f':
    ...
    if (!(s->flags & JSON_VALIDATE_ONLY))
    {
      ...
      if (root) *target = json_real_value(root);
    }
  ...
  case 'F':
    ...
    {
      double *target = va_arg(*ap, double*);
      if (root) *target = json_number_value(root);
    }

    return 0;

  case 'O':
    if (root && !(s->flags & JSON_VALIDATE_ONLY)) json_incref(root);
    /* Fall through */
